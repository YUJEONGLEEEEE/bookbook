        ...
        CoreDataManager.shared.fetchComments { [weak self] comments in
            ...
            if let next = BookReward.next(after: comments.count) {
                ...
                if let latest = earned.last {
                    ...
                    // 책 획득 X: 첫 책 안내
                    pool.append("책한줄을 작성하고 첫 책을 받아보세요!")
                }
                phrase = pool.randomElement() ?? pool[0]
            } else {
                // 모든 레벨 완료(최종 백과사전 획득) → 고정 문구
                phrase = "백과사전 획득 성공!"
            }

            let bookImageName = earned.last?.imageName ?? ""   // 카드 책 이미지는 항상 최신 획득 책
            self.profileView.configure(nickname: nickname, phrase: phrase, promise: promise, bookImageName: bookImageName)
        }
    }

    private func configureUI() {
        view.addSubviews([profileView, menuStackView, separator, tableView])
